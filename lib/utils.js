...
function env(prod, other) {
  if (other && _.isUndefined(process.env[other])) throw new CustomError(`Undefined env:${other}`);
  if (_.isUndefined(process.env[prod])) throw new CustomError(`Undefined env:${prod}`);
