...
// 给模型挂载子查询功能
function mountSubQuery(extendModel) {
  extendModel.subQuery = function(findJson) {
    const { model, prefix, foreignKey, targetKey } = findJson;
    const sequelize = extendModel.sequelize.Sequelize;
    const newFindJson = _.pickBy(findJson, (obj, key) => {
      return !([ 'model', 'prefix', 'foreignKey', 'targetKey', 'where' ].includes(key));
    });
    const where = foreignKey ? sequelize.literal('`' + extendModel.tableName + '`.`' + foreignKey + '`=`' + model.tableName + '`.`' + (targetKey || foreignKey) + '`') : {};
    if (findJson.where) {
      newFindJson.where = {
        [sequelize.Op.and]: [ where, findJson.where ],
      };
    } else {
      newFindJson.where = where;
    }
    const tempSQL = extendModel.sequelize.dialect.QueryGenerator.selectQuery(model.tableName, newFindJson).slice(0, -1); // to remove the ';' from the end of the SQL
