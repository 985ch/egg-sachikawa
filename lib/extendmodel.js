...
module.exports = function(model, defaultAttributes, needParse) {
  ...
  // 简化参数的findAll
  model.simpleFind = async function(where, attributes) {
    ...
    const rows = model.findAll(findJson);
  ...
  // 简化参数的findOne
  model.simpleFindOne = async function(where, attributes) {
    attributes = attributes || defaultAttributes;
    const findJson = {
      attributes,
      where,
    ...
    const obj = model.findOne(findJson);
